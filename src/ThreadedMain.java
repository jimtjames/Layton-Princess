...
public class ThreadedMain extends Thread {
    ...
    public static PiecePart[][] board;
    public static Piece[] pieceArr;
    public static ConcurrentLinkedQueue<StringBuilder> paths;
    public static int num_waiting = 0;
    public static final ReentrantLock lock = new ReentrantLock();
    public static int num_threads;

    public static void main(String[] args) throws InterruptedException {
        board = new PiecePart[4][5];
        ...
        pieceArr = new Piece[]{green1, green2, green3, green4, blue1, blue2, blue3, blue4, purple, red};
        ...
        num_threads = 4;
        ThreadedMain[] threads = new ThreadedMain[num_threads];
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new ThreadedMain();
    ...
    public void run() {
        ...
        while (!stop_running) {
            ...
            //if its null, its empty and we should just try again
            //note that this is busy waiting and probably not great
            //but we expect to not have an empty queue very often...
            //if all 4 are waiting, we should just give up
            //this seems really bad
            if (path == null) {
                System.out.println(this.getName()+" waiting...");
                //only modify num_waiting and such if this is the first time we are waiting
                if (!waiting) {
                    lock.lock();
                    num_waiting++;
                ...
                //if we were previously waiting lets update this
                if (waiting) {
                    lock.lock();
                    num_waiting--;
                    lock.unlock();
            ...
            if (count++ % 10000 == 0) {
                //System.out.print(".");
                System.out.println(this.getName()+" - len: " + (path.length() / 2) + " count: " + count);
                /*
                System.out.println("path: "+ path);
                try {
                    printBoard(execute_moves(path, board, pieceArr));
                ...
                for (int m = 0; m < 4; m ++) {
                    ...
                    try {
                        if (check_cycle_or_invalid(new_path, board, pieceArr)) {
                            ...
                            //printBoard(execute_moves(new_path,board,pieceArr));
                            paths.offer(new_path);
                        }
                    } catch (CloneNotSupportedException e) {
                        //should never happen, so fuck it
                        e.printStackTrace();
                        return;
                    }
                }
